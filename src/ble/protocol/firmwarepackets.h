...
//
// Three wire formats, all derived from de1app's binary.tcl spec:
//
...
//
//   Firmware chunk (A006, write, piggy-backed on WriteToMMR) — 20 bytes
//     0:    Opcode (0x10 selects firmware write)
//     1-3:  Address (u24, LITTLE-endian)
...
inline QByteArray buildChunk(uint32_t address, const QByteArray& payload16) {
    ...
    packet[0] = char(0x10);                                     // firmware-write opcode
    packet[1] = static_cast<char>(address & 0xFF);              // LE byte 0
    packet[2] = static_cast<char>((address >> 8) & 0xFF);       // LE byte 1
    packet[3] = static_cast<char>((address >> 16) & 0xFF);      // LE byte 2
