...
// File-scope helper: one English phrase for an equipment package — grinder,
// basket and puck prep, the three components a package forks on. Rendered at
// two sites (the hoisted `### Setup:` header and the no-history line), so it
// lives here rather than being written twice: the two would be free to drift,
// and a reader comparing them has no way to tell which one is stale.
//
// Segments are built as complete fragments and joined with " " so no segment
// owns a leading space and an absent field can't produce a double-space
// artifact (burrs without a grinder brand+model used to render "  with 63mm").
static QString describeEquipmentSet(const QString& grinderBrand, const QString& grinderModel,
                                    const QString& grinderBurrs, const QString& basketBrand,
                                    const QString& basketModel, const QString& puckPrep)
{
    QStringList parts;

    QString grinderName;
    if (!grinderBrand.isEmpty()) grinderName = grinderBrand;
    if (!grinderModel.isEmpty()) {
        if (!grinderName.isEmpty()) grinderName += " ";
        grinderName += grinderModel;
    }
    if (!grinderBurrs.isEmpty()) {
        grinderName += grinderName.isEmpty() ? grinderBurrs : " with " + grinderBurrs;
    }
    if (!grinderName.isEmpty()) parts << grinderName;

    QString basketName;
    if (!basketBrand.isEmpty()) basketName = basketBrand;
    if (!basketModel.isEmpty()) {
        if (!basketName.isEmpty()) basketName += " ";
        basketName += basketModel;
    }
    ...
    // stored canonically. It is part of the package identity, so it belongs in
    // the same sentence as the gear it was recorded with — see the equipment
    // -bucket note in shothistorystorage.h for the fork/merge rules.
    ...
    const QString& profileName, int excludeShotId, EquipmentScope* outScope)
{
    ...
    const bool opened = withTempDb(dbPath, "ai_context", [&](QSqlDatabase& db) {
        ...
        // Only asked when it can change what the model is told: on bucket 0 the
        // filter is a no-op for a user with no packages and a total exclusion
        // for a user with some, and the block cannot say which without this.
        if (scope.bucket == 0) {
            QSqlQuery pkg(db);
            if (pkg.exec(QStringLiteral("SELECT EXISTS(SELECT 1 FROM equipment_packages)")) && pkg.next())
                scope.userHasPackages = pkg.value(0).toBool();
            else
                qWarning() << "AIManager::requestRecentShotContext: package-census query failed:"
                           << pkg.lastError().text();
        }
        ...
        // Unconditional: bucket 0 is a real, matching bucket holding every
...
void AIManager::requestRecentShotContext(const QString& beanBrand, const QString& beanType, const QString& profileName, int excludeShotId)
{
    ...
    QThread* thread = QThread::create([self, dbPath, beanBrand, beanType, profileName, excludeShotId, serial]() {
        ...
        const bool ctxOpened = withTempDb(dbPath, "ai_grinder_ctx", [&](QSqlDatabase& db) {
            QSqlQuery q(db);
            ...
            if (!q.exec()) {
                ...
                if (!model.isEmpty()) {
                    // Same guard, same reason, as the recentAdvice block below:
                    // bucket 0 is a REAL bucket, so an unknown one would scope
                    // this to the unpackaged pool rather than decline to answer.
                    if (scope.bucketKnown)
                        grinderCtx = ShotHistoryStorage::queryGrinderContext(
                            db, model, bev, QString(), scope.bucket);
                    // Resolves its own scoping from the shot id, so it needs no bucket.
            ...
            if (!profileKbId.isEmpty() && scope.bucketKnown) {
        ...
        if (!ctxOpened) {
            // withTempDb does not run the lambda when the open fails, so grinder
            // context, calibration, recentAdvice and the equipment label are all
            // silently absent. Say so rather than let the payload read as "this
            // user has no observed grind settings".
            qWarning() << "AIManager::requestRecentShotContext: grinder-context connection "
                          "failed to open — no grinder context, calibration or recentAdvice";
            scope.historyReadable = false;
        }
...
static QString emptyHistoryReason(const EquipmentScope& scope)
{
    ...
    if (!scope.bucketKnown)
        // Reachable only when the shot's row was not FOUND: a query error or a
        // failed open clears historyReadable and is caught above. The candidate
        // query never ran, so nothing was scoped and nothing was excluded --
        // do not claim otherwise, which an earlier draft of this text did.
        return QStringLiteral(
            "This shot could not be found, so its equipment is unknown and no prior "
            "shots are listed here. Do NOT conclude that the user has no history.");
    if (scope.bucket == 0) {
        // This shot is on no package. Whether that filtered anything depends on
        // the USER, not on the shot: with no packages at all it is a true no-op
        // and there is nothing to say, but with packages it just excluded every
        // packaged shot they own -- which, said as nothing, is the "user has no
        // history" reading this block exists to prevent.
        if (!scope.userHasPackages)
            return QString();
        return QStringLiteral(
            "No prior shots on this bean and profile with no equipment package recorded. "
            "This user DOES have equipment packages, and shots pulled on them are "
            "deliberately excluded here because history is matched on the package. Judge "
            "this shot on its own curve and taste.");
    }
    ...
    const EquipmentScope& scope)
{
    ...
    QString setupBasketBrand, setupBasketModel, setupPuckPrep;
    ...
    for (const auto& qs : qualifiedShots) {
        ...
        // Basket and puck prep are part of the equipment set the history is now
        // filtered to, so they are constant across the block by construction and
        // hoist like the grinder does. Naming them matters as much as filtering
        // on them: a filter the advisor cannot see is a silent one, and the model
        // has no way to say "your Graph basket shots" — or to notice it is being
        // asked to compare a dial across two baskets — unless the payload says
        // which basket these shots used.
        seedOrCompare(setupBasketBrand, s.basketBrand);
        seedOrCompare(setupBasketModel, s.basketModel);
        seedOrCompare(setupPuckPrep, s.puckPrep);
    ...
    if (!shotSections.isEmpty()) {
        ...
        if (setupShared && (!setupGrinderBrand.isEmpty() || !setupGrinderModel.isEmpty()
            ...
            const QString gear = describeEquipmentSet(setupGrinderBrand, setupGrinderModel,
                                                      ...
                                                      setupBasketModel, setupPuckPrep);
            if (!gear.isEmpty()) parts << gear;
...
{
    // Clamp here, the last point every caller passes through. "No package" is
    // stored as SQL NULL and COALESCEd back to 0, so anything below zero has no
    // meaning and must fold into bucket 0 rather than hash a thread nothing finds.
...
void AIManager::indexStoredConversation()
{
    ...
    // Gate on the thread actually being on disk. Called both from the
    // conversationPersisted() wiring and directly from switchConversation(),
    // where the key may name nothing at all — indexing that would bring the
    // ghost entries back through a different door (see touchConversationEntry
    // in aimanager.h).
    AppSettings settings;
    // Classify rather than test bytes or bare-parse — see
    // AIConversation::storedTranscriptState for why those two are both wrong.
    // A CORRUPT thread is still indexed: `ai_conversations list` reports it with
    // `corrupted: true`, and a row saying something is wrong beats the thread
    // vanishing from the list entirely (the #1500 silent-failure finding).
    const AIConversation::TranscriptState onDisk =
        AIConversation::storedTranscriptState(settings, key);
    if (onDisk == AIConversation::TranscriptState::Missing
        || onDisk == AIConversation::TranscriptState::Empty)
        return;
...
void AIManager::makeRoomForNewConversation()
{
    ...
    while (m_conversationIndex.size() >= MAX_CONVERSATIONS) {
        ...
        qInfo() << "AIManager: Evicted oldest conversation" << oldest.key
                ...
                // "unknown", not 0: outTurns is filled on Ok alone, and 0 would
                // read as "nothing was lost" at the moment a transcript died.
...
void AIManager::clearAllConversationsOnce(const QString& migrationId)
{
    ...
    // INFO before the stamp, not after: the removals above have ALREADY run, so
    // logging only on the success path loses the record on exactly the launch
    // where data was destroyed and something then went wrong.
    ...
    if (!hadKeyedThreads && !hadLegacyThread)
        qDebug() << "AIManager: no conversations to clear for migration" << migrationId
                 << "— marker stamped so it does not run again";
}
...
QString AIManager::switchConversation(const QString& beanBrand, const QString& beanType,
                                      const QString& profileName, qint64 equipmentId)
{
    // Clamp here as well as in conversationKey, so the hash and the index entry
    // cannot disagree: clamping only inside the hash would leave
    // ConversationEntry::equipmentId holding the raw negative under a key
    // computed on 0. This is a Q_INVOKABLE, so the argument is whatever QML
    // passed.
    if (equipmentId < 0) equipmentId = 0;
    ...
    // Index only if the key really has a thread behind it — the gate inside is
    // what keeps this from manufacturing ghost entries, and what lets an
    // MCP-written thread (which the in-app index never saw) be picked up on the
    // first switch to it. See aimanager.h.
    indexStoredConversation();
...
void AIManager::loadMostRecentConversation()
{
    ...
    // The live identity has to come along, or a later Clear-then-send on the
    // restored thread re-indexes it with a blank bean/profile: indexStoredConversation()
    // builds its entry from m_live*, and the key alone is a hash it cannot reverse.
    m_liveBeanBrand = entry.beanBrand;
    m_liveBeanType = entry.beanType;
    m_liveProfileName = entry.profileName;
    m_liveEquipmentId = entry.equipmentId;
