...
- **Adding a log line? Read `docs/CLAUDE_MD/LOGGING.md` first — it is a gate, not advice.** `scripts/check_log_markers.py` fails the build-free PR job on a bare `qDebug` in a covered file, a marker typed by hand, or any leading `[Bracketed]` token the registry does not declare. Three things that keep going wrong and are all in that doc:
  - **The tier is chosen by AUDIENCE, not importance.** `INFO` is the user-visible tier — the connections views and `debug_get_log` run `marker + minLevel INFO` — so a user-facing line left at `DEBUG` vanishes. The recurring shape is a fault reported at `WARN` whose *resolution* sits at `DEBUG`: the reader sees only the failure half and concludes it never recovered.
