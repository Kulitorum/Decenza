...
- **A test has to be able to fail, and it costs BUILD time, not run time.** The suite runs in ~31 s; an individual test is 50-80 ms. That is noise. The cost is that there are already 106 test targets, each a compile *and* a link against Qt and `decenza_shotlib` on every build — so **add to an existing target whose fixtures fit, and only create a new one when none does.** A tidier filename is not a reason. Before keeping a test, break the code it covers and watch it go red: a test that cannot fail is a comment that compiles, and deleting one is a strict improvement. Full rules, and the three pass-forever shapes that have shipped here, are in `docs/CLAUDE_MD/TESTING.md` under "Adding New Tests".
