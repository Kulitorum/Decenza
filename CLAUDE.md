...
- **Never use timers as guards/workarounds.** Timers are fragile heuristics that break on slow devices and hide the real problem. Use event-based flags and conditions instead. For example, "suppress X until Y has happened" should be a boolean cleared by the Y event, not a timer. Only use timers for genuinely periodic tasks (polling, animation, heartbeats), **UI auto-dismiss** (toasts/banners that hide after N seconds), and **a hardware signal whose DURATION is the discriminator**. Everything else — including debounce — should use event-based flags.
  ...
  The hardware carve-out is narrow and has to be argued, not assumed: it applies when the machine reports the same value for a real condition and for a transient, so no snapshot can tell them apart and no event says "still true" — the DE1 emits `Error_NoAC` identically for an open standby switch and for a few seconds while it wakes, and pushes nothing while a condition merely persists (`machinestate.cpp`). Reaching for history instead is what fails there: keying on the preceding substate looked event-based and missed the commonest path, because the substate it arrives from differs per entry point. When the carve-out applies, prefer de1app's interval for the same signal over inventing one — Decent write the firmware that emits it.
