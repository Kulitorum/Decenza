...
- **New layout widgets** require registration in 4 places: (1) `CMakeLists.txt` file list, (2) `LayoutItemDelegate.qml` switch, (3) `LayoutEditorZone.qml` widget palette + chip label map, (4) `shotserver_layout.cpp` web editor widget list. Optionally add to `LayoutCenterZone.qml` if the widget should be allowed in center/idle zones. If the widget has per-instance options, declare its option keys in the readout capability schema (`readoutOptionSchema()` in `settings_network.cpp`) — the in-app gear indicator, the unified `ReadoutOptionsPopup`, and the web editor's option forms all derive from that one table.
