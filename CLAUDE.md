...
- **Keep main-thread database and disk I/O off anything a user can feel, and off anything that could interfere with making coffee.** That is what the rule protects — it is not a ban on touching `QSqlQuery` from the main thread, and reading it that way produces caches far more expensive than the thing they avoid. Judge by magnitude and by where the work sits:
  ...
  - **Inline is fine** for a small bounded read on a discrete user action — opening a picker or a dialog. Measure it on a realistic database and on a multiple of one (a scan's cost tracks table BYTES, not row count: a page read drags the blobs along), then put the median AND the worst case in a comment at the call site so the next person inherits the evidence rather than the conclusion.
  ...
  - **The tradeoff runs both ways, and the expensive side is usually the cache.** `grindStepForGrinder()` was cached precisely to keep a QML binding off the database, and the cache — resident value, invalidation, supersession guard, failed-key set — is what broke in #1724: the step derived correctly as 0.25, an invalidation dropped it, the re-fetch was discarded mid-flight, and the picker showed 1.0 for the rest of the session. Reading live is 3.3 ms median / 87 ms worst on a real 18.5 MB database and 37 ms median on a 16× copy. Pay for threading or caching where a user or the machine would notice. Nowhere else.
...
- **Complexity has to come with a measurable win, stated in units the USER feels.** Not "33.9 ms → 1.18 ms" — *who waits for those milliseconds?* A saving on a background thread, at startup, that nothing blocks on is not a win, however large the ratio. Write the benefit as "X, which the user experiences as Y" before adding the machinery; if Y is empty, so is the case for it. This is the check the rest of the process does not do: build, tests and the PR review agents all ask whether code is CORRECT, and over-built code almost always is — a covering index nobody needed passed 110 tests and was praised in review.
  - **Re-derive the justification when the design around it moves.** The index above was justified by "a visible hitch every time the grind picker opens", which was true of an on-demand read and false the moment the value became resident. The comment was never revisited, so a stale premise kept a schema migration alive through a full review. When you change how something is read or called, re-read the comments justifying its optimisations — they are claims with expiry dates.
  - **Needing fault injection to reach a branch is a stop sign, not a testing problem.** If the only way to exercise a path is to fake a failure that cannot otherwise occur, ask whether the path should exist before you build the harness. Writing exactly that harness is what exposed the over-build described in the migration rule below.
- **Don't engineer around migrations. They run once, with the app stopped.** Every migration executes synchronously inside `ShotHistoryStorage::initialize()`, which does not set `m_ready` or emit `readyChanged()` until the whole chain is done — so no reader, background thread or UI, can ever observe a half-migrated database. That means the concurrency defences a migration seems to invite are guarding nothing: don't add "is a migration running" flags, don't make readers skip or wait, don't build retry/verify machinery for a state no one can see. Just do the work and stamp the version.
  - **Gate the version bump only on things that are schema facts.** A column, a table, a data backfill — bump only if it landed, because the app would be wrong without it. A performance index is not one of those: the queries return identical results without it, so create it, log the failure if it fails, and bump anyway. Migration 36 was first written to verify its covering index by column list and hold the version at 35 to "retry next launch" — machinery to protect a reader that does not exist, at the cost of a database that fails `CREATE INDEX` once re-running the block on every launch forever. The stamp of `schema_version` itself stays transacted and checked; that one is a real correctness fact (see #1586).
  - The corollary is a review smell: **if a migration needs fault injection to test a branch, ask whether the branch should exist.** Writing that test is what exposed the over-build above.
