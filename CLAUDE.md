...
That is narrower than "there is no PR CI", which this file used to say and which is wrong: `text-invariants.yml` **does** run on `pull_request`, filtered to `qml/**`, **`src/**`**, `scripts/check_*.py`, `tests/CMakeLists.txt` and `resources/fonts/**`. **`src/**` is in that list**, so a pure C++ change is gated too — this file previously omitted it, and that omission is what let a merge land red: `check_log_markers.py` rejected a new `[Equipment]` line in `maincontroller.cpp` on the PR, the PR was merged without reading its own run, and `main` went red. Read the run, not this list. It is build-free by design — pure Python over the source, no Qt, no compile, ~1 minute — which is exactly why it can afford to run per-PR while nothing else does. Read it as the shape a new PR-time check has to fit: if a check needs the app built, it does not belong there.
