...
That is narrower than "there is no PR CI", which this file used to say and which is wrong: `text-invariants.yml` **does** run on `pull_request`, on **every** PR, and it is a **required status check** — a red one blocks the merge button.

It used to be path-filtered, and this file listed the filter while omitting `src/**`. That omission is what let a merge land red: `check_log_markers.py` rejected a new `[Equipment]` line in `maincontroller.cpp`, the PR was merged without its own run being read, and `main` went red. The filter is now gone entirely — a required check that is path-filtered never reports at all on a PR outside the paths, so it hangs pending and cannot be merged. Do not reintroduce one. It is build-free by design — pure Python over the source, no Qt, no compile, ~1 minute — which is exactly why it can afford to run per-PR while nothing else does. Read it as the shape a new PR-time check has to fit: if a check needs the app built, it does not belong there.
