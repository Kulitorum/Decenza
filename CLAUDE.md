...
### Design Principles
- **Never use timers as guards/workarounds.** Timers are fragile heuristics that break on slow devices and hide the real problem. Use event-based flags and conditions instead. For example, "suppress X until Y has happened" should be a boolean cleared by the Y event, not a timer. Only use timers for genuinely periodic tasks (polling, animation, heartbeats), **UI auto-dismiss** (toasts/banners that hide after N seconds), and **a hardware signal whose DURATION is the discriminator**. Everything else — including debounce — should use event-based flags.

  The hardware carve-out is narrow and has to be argued, not assumed: it applies when the machine reports the same value for a real condition and for a transient, so no snapshot can tell them apart and no event ever says "still true". The DE1 emits `Error_NoAC` identically for an open standby switch and for a few seconds while it wakes, and `DE1Device` emits `subStateChanged` only when the value CHANGES (`de1device.cpp:705`, emitted at `:796-797`), so a condition that merely persists generates nothing to listen for. Claim that, not something about the machine's own push cadence — `BLE_PROTOCOL.md` records that as unmeasured, and an earlier version of this line asserted it anyway.

  **This is not the debounce forbidden above, and the difference is the point.** Debounce filters a signal whose transitions we can see and count: the edges are the evidence, so a flag driven by them works. Here the two cases produce the SAME value with no distinguishing edge at all, so there is nothing for a flag to key on. Reaching for history rather than duration is exactly what failed in `machinestate.cpp`: keying on the substate an episode arrived from looked event-based and missed the commonest path, because that substate differs per entry point. If you can name the event, you do not have this exception.

  Pick the interval from measurement and say whose it is. Do not borrow a number from another app without reading what its timer actually does — that mistake was made here, citing a de1app `after 6000` that is a one-shot post-connect re-check, not a settling wait, and it reached a rule file and two spec documents before anyone opened the source.
