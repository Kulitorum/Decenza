...
- **Stop limits**: `target_weight` (SAW) and `target_volume` (SAV) are checked independently — whichever triggers first stops the shot. A value of 0 means disabled. SAV uses a raw `pourVolume >= target` comparison with no lag compensation (matching de1app — volume is already imprecise from the flow sensor). SAW ignores the first 5 seconds of extraction to avoid false triggers from tare settling and puck swelling (matching de1app). For **basic profiles** (`settings_2a`/`settings_2b`) with a BLE scale *configured* (not just connected), SAV is skipped (matching de1app's `skip_sav_check` / `expecting_present`) because basic profiles often have unrealistically low volume targets. The DE1 firmware also has a `TargetEspressoVol` safety limit (200 ml, matching de1app's `espresso_typical_volume`) sent via `setShotSettings` — this is a high safety net, not an active stop condition.
