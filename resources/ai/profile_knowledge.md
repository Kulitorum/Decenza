...
## D-Flow
UGS: 0.5 (the grind center for "D-Flow / default"; there is no "D-Flow" profile, D-Flow is the editor. "D-Flow / Q" is split to the "D-Flow Q variant" section at a coarser ~1.0; "D-Flow / La Pavoni" is split to the "D-Flow La Pavoni variant" section at a coarser ~1.0; "Damian's LRv2/LRv3" to their own section at 0)
Also matches: "D-Flow / default", "Damian's D-Flow", "Damian's LM Leva"
AnalysisFlags: flow_trend_ok
Category: Lever/Flow hybrid (Londinium family)
Family: lever-decline
How it works: Pressurized pre-infusion with soak, then flow-controlled pour (default 1.7 ml/s). Pressure peaks (~8–9 bar) then gradually declines — intentional lever-style behavior. Can heal uneven puck prep. D-Flow is a Recipe Editor *type*, not a profile. **Only a profile whose title starts with "D-Flow/" was created with the D-Flow editor** (the profile is the name past the "/", e.g. "D-Flow / default", "D-Flow / Q"). This pressurized-soak → flow-pour structure, the lever-style pressure decline, and the per-profile pressure-limit clamp are editor-level behavior shared by every "D-Flow/"-titled profile. "D-Flow / Q" and "D-Flow / La Pavoni" each use a distinct pressure target and fill temperature and are covered in the separate "D-Flow Q variant" and "D-Flow La Pavoni variant" sections. The other profiles this section also matches by alias — "Damian's LM Leva", "Damian's D-Flow", and (in their own section) "Damian's LRv2"/"Damian's LRv3" — are standalone profiles by the same author (Damian Brakel), each a distinct profile, NOT D-Flow-editor output; they are grouped with D-Flow only for shared lever-decline behavior.
Profiles in this section: "D-Flow / default" is the starter/example D-Flow-editor profile. ("D-Flow / La Pavoni" is a distinct D-Flow-editor profile — 84°C fill, 6–9 bar dial-in target — split to the "D-Flow La Pavoni variant" section.) "Damian's LM Leva" is a standalone profile by Damian (not D-Flow-editor output) — a pressure-profile recreation of a real La Marzocco Leva recording.
...
UGS: ~1.0 (inferred — La Pavoni's 6–9 bar pressure target + 84°C fill pull coarser than D-Flow / default's ~9 bar/88°C, the same mechanism the "D-Flow Q variant" section documents; not on the UGS chart)
Also matches: "D-Flow / La Pavoni"
AnalysisFlags: flow_trend_ok
Category: Lever/Flow hybrid (Londinium family)
Family: lever-decline
How it works: This section covers "D-Flow / La Pavoni" (created with the D-Flow editor). Damian created it to simulate the results from his La Pavoni lever machine, which he profiled side by side with a Slayer V3 for the same taste in cup. It uses an 84°C fill temperature and a lower pressure target than D-Flow / default, and produces the same lever-decline shape (pressurized soak → flow-controlled pour, declining pressure) — see the "D-Flow" section for that shared behavior and the DO NOT flag list, all of which apply here too.
Stated dial-in target (profile notes, verbatim): "The goal is to grind for a pressure peak between 6 and 9 bar." Aim for the pressure peak to land within ~6–9 bar.
Why the grind differs from D-Flow / default: the lower 6–9 bar target wants a coarser grind than D-Flow / default's ~9 bar peak; the low 84°C fill reduces solubility and pulls slightly back finer. Net: noticeably coarser than D-Flow / default — do NOT transfer a D-Flow / default grinder setting 1:1 to this profile.
Setup: 18g VST basket; parameters tuned for milk drinks; emulates the La Pavoni's pressure curve/timing against a Slayer V3 reference. de1app stock parameters: fill 1.2 bar, 84°C fill, pour flow 2.4 ml/s, pour pressure limit 9.0 bar, weight 46 g.
Expected curves: Pressure peaks within ~6–9 bar, then declines (lever-style). Declining pressure and slow 0–0.4 ml/s flow during the early soak are NORMAL and INTENTIONAL.
DO NOT flag the large gap between a high rise setpoint and the lower actual basket temperature as a problem — the coffee never reaches the setpoint; the high setpoint just drives the heater pump. It is by design.
...
Family: gentle-long-preinfusion
