...
- [x] 3.1 `qml/components/BagCard.qml:454` — change `visible: !card.isFrozen` to a predicate meaning "no portion currently in the freezer": true when `frozenDate` is empty OR `defrostDate` is set. Add it as a `readonly property bool` next to `isFrozen` (`:28`) with a name that says what it means (e.g. `portionInFreezer` and use `!portionInFreezer`) — do NOT redefine `isFrozen` itself, which also drives the "Frozen" badge and the freeze toggle's checked state where "has ever been frozen" is the intended reading
