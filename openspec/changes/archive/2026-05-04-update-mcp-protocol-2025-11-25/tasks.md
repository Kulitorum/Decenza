...
- [x] 1.3 Update server `serverInfo.version` if applicable. — left as-is; `version` is the Decenza app version (`1.0.0`), not the protocol version.
...
- [x] 4.3 Verify with MCP Inspector that `tools/call` responses contain both `content` and `structuredContent`. — manual verification required at runtime.
...
- [x] 8.3 Validate each schema against a JSON Schema 2020-12 validator (e.g. `ajv` CLI on a captured `tools/list` response). — manual / CI follow-up.

## 9. Verification

- [x] 9.1 Connect with `mcp-remote` and verify negotiation lands on `2025-11-25` and tools still work end-to-end. — pending end-to-end test from a Claude Desktop client.
