...
- [x] 11.2 Run the full test suite via Qt Creator MCP. — *not run via Qt Creator MCP (this worktree wasn't open as a Qt Creator project), but run for-real via a manual `linux-release.yml` CI dispatch (2026-07, run `29039076125`): full build + `ctest` all-green, 64/64 tests passing, including the new `tst_kb_schema` and `tst_dialing_blocks` additions from this same audit.*
