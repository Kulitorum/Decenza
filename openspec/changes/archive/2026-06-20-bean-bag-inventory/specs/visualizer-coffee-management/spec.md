...
All endpoint behaviors below are spike-verified against the live Visualizer API and the
open-source controllers (design.md "spike findings", 2026-06-11). Key facts the design
rests on: bag CRUD is premium-gated (not CM-gated); `coffee_bag_id` on shot PATCH is
only permitted when the user's `coffee_management_enabled` flag is on (single-key PATCH
→ 400 when off, 200 when on); `GET /api/coffee_bags` non-empty does NOT imply CM on;
the upload POST ignores `coffee_bag_id`; linking observably rewrites the shot's bean
fields server-side.
