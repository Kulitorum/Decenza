...
- [ ] 7.3 Android verification per the platform-code rule — CI test build (`gh workflow run android-release.yml --ref <branch> -f upload_to_release=false`), then confirm on device that settings survive and only one `.conf` remains
- [ ] 7.4 Open the PR from a feature branch; review before merge
- [ ] 7.5 `openspec archive consolidate-qsettings-stores` as the final commit on the branch
