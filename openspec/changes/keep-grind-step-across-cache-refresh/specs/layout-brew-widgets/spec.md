...
Each section's step between candidate values SHALL be derived from the user's own shot history rather than a user-configured constant: the Grind step from the grinder's observed settings, the RPM step from the grinder's observed RPMs, via the same noise-filtered estimator. The widget SHALL request each step from the shot-history store scoped to the currently selected grinder, and SHALL NOT read any `grindQuickSelectStep` setting (that setting is removed by this change).

When history is too thin to derive — fewer than two distinct numeric values — the Grind step SHALL default to `1.0` and the RPM step to `50`. Those defaults describe **thin history and nothing else**. They SHALL NOT be shown while a step is derivable for that grinder: a store that holds the history but has momentarily lost its cached copy is not thin history, and a widget standing on the default in that state is a defect, not the specified fallback. The step SHALL therefore be available to the widget synchronously at the moment it builds its rows (see `grind-step-derivation`), rather than arriving after an asynchronous round-trip the widget has already rendered without.
...
#### Scenario: The default is not shown while a step is derivable

- **GIVEN** a grinder with 28 distinct observed settings deriving a step of `0.25`
- **WHEN** the widget builds its rows immediately after a shot save, a shot edit or an equipment change has invalidated shot history
- **THEN** the offered values SHALL step by `0.25`
- **AND** the widget SHALL NOT step by `1.0`
- **AND** a recorded value of `7.75` SHALL NOT be reformatted to `8`
