...
2. **Origin allowlist locks out legitimate browser clients.** Mitigation: empty-Origin requests (CLI tools, mcp-remote) are unaffected, and the settings UI exposes an additional-origins field for advanced users.
