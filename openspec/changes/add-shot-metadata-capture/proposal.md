...
- **NEW** `parseBeanCorrectionsFromReply` static parser on `AIManager` — scans a user reply for explicit, conservative bean-field corrections and returns a sparse `BeanCorrection` struct. Detects `roastLevel` (Light / Medium-Light / Medium / Medium-Dark / Dark), `beanBrand`, `beanType`, `roastDate` (ISO and a small set of natural-language date forms). Rejects compound phrases ("dark chocolate note", "light fruit") to keep false positives near zero.
