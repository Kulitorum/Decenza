...
- "Selected" is two settings, `selectedBuiltInProfiles` (opt-in) and `hiddenProfiles` (opt-out), read through `ProfileManager::isProfileInSelectedList()`. The auto-load gate (`loadAutoLoadProfileIfNeeded`), MCP `auto_load` and `addFavoriteProfile` (which un-hides and selects) depend on it. Unchanged.
...
**D8. Selected toggle in ⋮ only.** Keeps card hit targets at five. Toasts and `AccessibilityManager.announce` pairs move into the dialog handler unchanged.
