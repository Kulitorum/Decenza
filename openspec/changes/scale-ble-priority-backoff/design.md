...
*Why:* The skip-HIGH decision is latched app-run-wide on the `BLEManager` singleton (see D5), so it survives both the backoff-induced reconnect and a scale-type change (which builds a fresh transport). "Skip HIGH ⇒ BALANCED" is not new code — it is the exact path #1097 already ships and proved good. This deletes the live-renegotiation + confirmation state machine and the new coordinator class entirely. Scale-agnostic: `QtScaleBleTransport` is shared by all 14 scale drivers, and the latch is shared across all of them.
...
### D5: App-run-scoped backoff (BLEManager latch) — no disk persistence
The skip-HIGH decision is latched on the process-lifetime `BLEManager` singleton, not on the per-scale transport. Detection runs at most once per app run: the first scale to prove the device can't sustain dual-HIGH sets the latch, and from then on **every** scale this run (including one connected after a scale-type change, which builds a fresh transport + detector) skips HIGH. The latch is in-memory only — never written to disk/settings — and is cleared by an application restart, which re-runs detection from scratch.

*Why:* The contention is a property of this device's BT radio + the DE1 link, **not** of any one scale (the fix is scale-agnostic; #1093 was a Eureka Precisa, #1176 a Decent). Scoping the latch to a single transport instance would make a scale-type change wrongly re-pay the ~1–15 s detection window (and risk a bad shot) on hardware already known-incapable this run. BLEManager is the natural home: process-lifetime, already referenced from the transport, survives transport recreation, in-memory. No disk persistence avoids a settings field, lock-in of a misclassification across runs, and any stale-classification path when hardware/RF conditions change between runs. A false-positive costs at most one app run at BALANCED for all scales and self-corrects on restart.

*Alternative considered:* persist to disk across runs (self-heal once, never re-suffer the window) — rejected: adds a settings field + corroboration + a re-probe/clear path to avoid permanent misclassification, and the cost it saves is one detection window per app run. Per-transport scoping (an earlier iteration) was also rejected: too narrow, re-pays detection on scale-type change.
...
The scale always requests HIGH initially (symmetric with the DE1) unless the app-run BLEManager latch is set, and observed runtime behavior (D1/D2) decides backoff. The `kMinSdkForScaleHighPriority` SDK-30 branch is removed entirely — observed behavior, not OS version, is the predictor: even genuinely old devices simply start HIGH and back off within the detection window once per app run.
...
- **Detector false-positive on capable hardware** (transient hiccup during a pour) → write-retry no longer counts (only write-failed/controller-error); with no disk persistence (D5) a false-positive is non-sticky — at worst one app run at BALANCED for all scales, self-corrected on restart.
...
- **Weak device re-pays the ~1–15 s detection window once per app run** (the explicit cost of D5) → Accepted; detection is fast and pre-shot in the observed timings, and the latch then covers all scales for the run. Revisit disk persistence only if the per-run window proves painful in practice.
...
- Sequencing: single PR — D1–D3 + D7 (app-run-scoped via BLEManager, no disk persistence, no live renegotiation). Cross-referenced to #1093/#1176; plain issue links until the reporter confirms.
...
- Whether the once-per-app-run re-detection window is acceptable long-term, or whether disk persistence (the rejected D5 alternative) should be reconsidered after real-world use.
