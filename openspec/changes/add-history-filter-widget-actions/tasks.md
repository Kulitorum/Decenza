...
- [ ] 6.6 In the running app: place a Custom widget per action, verify each opens History with the right banner and result set; verify the no-active-recipe / no-bean / no-bag / no-profile cases open unfiltered; verify a tap while already on History re-filters in place and that plain "Go to History" clears the filter
