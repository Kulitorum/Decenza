...
That state was previously recorded here as "NOT a property of the responder" and as a host-side condition cleared by a tablet reboot. **A two-scale control on 2026-08-06 refutes that reading.** In one window a tablet resolved, in 272 ms, the one scale it had just opened a TCP connection to, while a second scale on the same LAN — never contacted from that tablet — stayed silent to it and answered a Mac normally. One resolver stack, one moment, opposite outcomes per peer; no state of the tablet's resolver can produce that. The earlier reboot appeared to fix it because it was followed by traffic to the scale. What is established is that the responder answers **per peer**, and that having dialled a scale is what puts this device on its answering side.

The mechanism behind that is **suspected, not proven**. The fitting account is that a query from an ephemeral source port is a "legacy" query under RFC 6762 §6.7, which the responder must answer by unicast; to unicast it must resolve the querier's IP to a MAC, and ESP-IDF ships `ETHARP_TRUST_IP_MAC` off, so it cannot learn that MAC from the multicast query frame it just parsed. No ARP frames were captured and the scale's ARP table was never read. It also has a counter-example on the record: misses continuing against a scale that had served a WebSocket on its IP 16 s earlier, which a cold ARP entry does not explain. Nothing in this specification SHALL depend on that mechanism being correct.
