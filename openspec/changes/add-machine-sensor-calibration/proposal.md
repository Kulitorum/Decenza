...
- The wizard owns the whole loop for one sensor at a time (**pressure** or **temperature**): load the matching test profile, tell the user what hardware to fit, watch the run, capture what the machine's own sensor reported during the steady hold, take the external instrument's reading, write the correction, then offer a re-run so the user can watch the two converge.
- Because the wizard observes the run, the machine-reported half of the correction is **measured, never typed or inferred**. That removes the whole class of errors where a user submits a correction against a value the machine never reported.
