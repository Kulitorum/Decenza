...
- **One on/off toggle that preserves calibration.** A single switch ("Weight-timed steaming", default on) gates **all** scaling — when off, every path uses the preset's fixed duration. The per-pitcher calibration is retained, so turning it back on resumes immediately.
