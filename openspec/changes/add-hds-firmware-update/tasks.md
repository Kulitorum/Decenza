...
- [x] 4.6 Replace the device-display handoff copy: the dialog states that the update has started on the scale, never that it is installed, and no longer directs the user to the scale's display. Verify the started state and the absence of any completion claim.
