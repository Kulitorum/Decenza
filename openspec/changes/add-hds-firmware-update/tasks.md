...
- [ ] 5.2 Verify the install on hardware once upstream publishes a stable release newer than the scale under test, confirming no picker appears. The original blocker (catalog offering only newer releases, `releases/latest` skipping the `v3.1.14-preview.1` pre-release) is resolved — `v3.1.14` stable published 2026-09-16. BLOCKED again, differently: the test scale was updated to exactly `3.1.14` by task 5.3's run, so there is no newer release left to target until the next one ships. When unblocked, confirm on Bluetooth, USB, and WiFi that no picker appears, the scale reconnects on the target version, and a failed update leaves the installed firmware running.
