...
A restarted scan could re-discover the R2 while a previous instance is still connecting, and the `refractometerDiscovered` handler would tear it down and recreate. In practice this cannot loop: a scan cycle is ~15 s and an R2 connect completes in ~2 s, so by the next discovery the instance is connected and the handler's "same device already connected" guard short-circuits. A connect that takes >15 s is wedged, and teardown-and-retry is the desired recovery. No extra "connecting" state flag is added (minimal root-cause scope).
...
- [Hunt flag leaks on if a page teardown path misses deactivation] → Both `StackView.onDeactivating` and `Component.onDestruction` clear it; the guards (`connected`, no saved address, BLE disabled/unavailable) make a leaked flag inert in the common cases anyway.
