...
- [x] 4a.2 Compact-bar popups: `RecipesItem.qml`, `BeansItem.qml`, `EspressoItem.qml` — replace `pillPageSize: 5` slicing / unbounded wrap with a live two-row fit (measure with a `TextMetrics` mirroring `PresetPillRow`'s constants; pack against the child's real `effectiveMaxWidth`, two passes for the arrow gutter; maxRows = 2). Keep the existing `pageChangeRequested`/`selectedIndex`/`onPresetSelected` wiring; map profile pills' page-relative index to the absolute `selectedFavoriteProfile`.
- [x] 4a.3 Center-zone expansion: `IdlePage.qml` — same live two-row fit for the recipes, beans, AND favorite-profile rows, driving each off its Loader width (the pill-row id is Component-scoped). Reset each page index on activation; map the profile row's select/long-press to absolute indices; announce the visible page.
