...
- [~] 8.5 Visualizer mapping: `applyTasteCvaMapping` is a file-static pure lookup with no existing VisualizerUploader test harness — covered by the value-table review + manual verification rather than a disproportionate new harness. (Revisit if a uploader test harness is ever added.)
