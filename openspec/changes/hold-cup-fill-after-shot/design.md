...
The gate itself came from #855, whose comment names the case it was defending: a residual scale reading — notably a hot-water pour's frozen weight — drawing a full cup during `EspressoPreheating`, before any coffee exists. That defence must survive this change.
...
- Changing what `MachineState.scaleWeight` publishes. Other consumers (MQTT, widget snapshot, MCP surfaces, SAW learning) read it and must keep seeing the live value.
...
**Latch on `Preinfusion`/`Pouring`, not `EspressoPreheating`.** Preheat is a pre-flow phase; latching there would reintroduce the residual-weight case in full.
