...
- [~] 8.4 **Deferred (no harness; covered by construction).** No `registerDeviceTools` unit harness exists (the existing `devices_reset_scale_priority` likewise has none); the tool is thin validation + a marshalled `setBackoffMode`. The behavioral core is unit-tested (tst_scaleskiphighlatch mode persistence + detector observe tests); end-to-end is verification 9.2. Original: `devices_set_scale_priority_mode` rejects bad mode / missing confirm; `devices_connection_status` reflects mode + event list shape.
...
- [~] 9.1 **Deferred to maintainer device test.** Enforce parity is covered deterministically (tst_scaleblepriority `enforceUnchangedWhenNotObserving` / `observeAndEnforceShareTriggerPoint`; detector enforce path is byte-identical by construction — `m_observe` defaults false and the latch/disarm code is untouched). The live debug-log diff needs hardware running this build (the in-field app is v1.7.5, not this branch) → maintainer.
