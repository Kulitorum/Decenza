...
- [ ] 6.3 Exercise the ladder against the running app over MCP: activate a recipe, switch profiles, read the recipe's stored dose back — specifically including a RESTART in the middle, which is the case that was broken
...
- [~] 6.5 Wiki manual written and committed locally in `~/Development/GitHub/Decenza.wiki` (Manual's "Recommended Dose" + the matching FAQ entry, both of which said the profile's dose applies whenever the profile is selected; plus a new FAQ entry for the symptom users hit first). **NOT pushed** — the wiki is live and public, and publishing it before this PR merges would document behaviour nobody has yet. Push it with the merge
