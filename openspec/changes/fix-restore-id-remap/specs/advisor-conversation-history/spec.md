...
`shotId` binds a conversation turn to the shot it discussed. Importing a conversation alongside a shot history that is being renumbered SHALL rewrite each turn's `shotId` to the destination id of the same shot, so the binding continues to name the shot the turn is actually about.
