...
- **One list of bundled profiles**: `resources/profiles.qrc`, linked by the app and by the test targets. `resources.qrc` used to carry a second copy and the two drifted for two months — nine profiles (Adaptive v3 among them) reached every test binary and no release. `scripts/check_profile_resources.py` fails a PR when `resources/profiles/` and `profiles.qrc` disagree or another `.qrc` lists a profile. When de1app replaces a built-in in place under a new title (best_practice.tcl, Adaptive v2 → v3), retire the old file and add its successor to `kSuccessor` in `ProfileManager::refreshProfiles()` so favorites, current profile and auto-load follow it.
