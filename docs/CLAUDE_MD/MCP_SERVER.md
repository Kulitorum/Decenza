...
| `equipment` action=`update` | Edit a package's grinder identity (`grinderBrand/Model/Burrs`) and/or `name`. Partial; re-derives `rpmAdjustable`. Reference semantics (applies to all referencing bags and recipes). CHANGING a component on a package that has shots forks a new package (returned `package.id` differs) — bags and recipes move to it, the shots stay on the old identity; filling in a component that was EMPTY is enrichment and edits in place. | settings |
