...
| `machine_start` | Start an operation: `action` = `espresso` \| `steam` \| `hot_water` \| `flush`. No brew arguments: set dose, yield, ratio, temperature or grind with `settings_set` first (they persist). Every action raises the in-app dialog. | control |
...
| `equipment` action=`create` | Add a package from a grinder and/or basket identity (+ optional `name`, `puckPrep`) through `EquipmentStorage::createPackageStatic`, the Switch Equipment dialog's rule: identical gear returns the existing package, a taken name is refused. Not auto-selected. | settings |
