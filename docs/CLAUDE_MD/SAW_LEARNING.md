...
- Two reset actions in the same card:
  ...
  - **Reset all** — clears every SAW key (global pool, all per-pair history + batch, all bootstrap values, the basket-seed flag, hot-water SAW offset). **Behind a confirmation**, unlike the scoped resets: it discards months of learning for every profile and scale the user owns.
