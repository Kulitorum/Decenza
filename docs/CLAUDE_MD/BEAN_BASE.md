...
**The canonical id is authoritative for IDENTITY server-side, so the shot link is guarded.** `Shot#refresh_coffee_bag_fields` (`app/models/shot.rb:64-75`) fires on `canonical_coffee_bag_id_changed?` and overwrites `bean_brand` = `canonical_roaster.name` and `bean_type` = `canonical_coffee_bag.name`. A **borrowed** record — the same coffee scraped from another roaster, which is a legitimate local state because the bag editor keeps identity editable while linked — therefore RENAMED the user's shots on visualizer.coffee: a Stavanger Kaffebrenneri bag uploaded as "Coava Coffee Roasters", per-bean, so the roaster looked randomly inconsistent. `BeanBaseBlob::canonicalIdentityConflicts()` now gates both export sites (metadata PATCH and `linkShotCanonical`): it compares the blob's **pristine** names (the `canonical` snapshot where present — `roasterName`/`roastName` are user-editable) against the local ones, and an empty name on either side is not a conflict. The link stays local; only the export stops. The `coffee_bag` branch of the same callback takes the user's own bag roaster, so the bag-level canonical fields (`addBagDescriptiveFields`, `buildBagEnrichBody`) are unguarded on purpose. Existing shots do not self-heal — the server rewrites only on link change, so a shot renamed before the fix keeps its wrong roaster until something PATCHes it again.
