...
**Cache budget.** `pre-merge.yml` **restores** ccache on pull requests but only **saves** on `push` to `main`. GitHub scopes caches by ref and `prune-caches.yml` keeps one generation per (prefix, ref), so a PR that saves costs ~0.3 GB *per open PR* rather than ~0.3 GB total — measured at 0.60 GB from four generations of a single PR in one afternoon. The repo runs at roughly 7 GB of GitHub's 10 GB cap, and the least-recently-used entries are the tag-push ones (`qt-macos` 0.61 GB, `qt-android` 0.46 GB, `gradle` 0.36 GB), so unbounded PR churn would evict exactly the caches that keep **release** builds warm. PRs stay warm anyway because a pull request can read the default branch's cache scope. Both new jobs also set an explicit `max-size`.
